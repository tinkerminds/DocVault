...
**Security Note**: ⚠️ Stored locally only - never committed to Git
...
- [ ] Create Git branch: `feature/blob-upload-service`
- [ ] Scaffold .NET 8 Web API project
- [ ] Install `Azure.Storage.Blobs` NuGet package
- [ ] Create service layer:
  - [ ] `IBlobStorageService` interface
  - [ ] `BlobStorageService` implementation (with retry logic)
  - [ ] `BlobUploadResult` model
  - [ ] `AzureStorageOptions` config class
...
**Q: Why LRS instead of GRS (Geo-Redundant)?**  
A: For dev environment, LRS is sufficient. Production should use GRS.

**Q: Why separate container for uploads vs thumbnails?**  
A: Separation of concerns + different lifecycle policies (thumbnails can be deleted faster).

**Q: Can I change storage account name later?**  
A: No - storage account names are immutable. Must delete and recreate.

**Q: Where is the data physically stored?**  
A: Central India region has multiple availability zones. LRS keeps 3 copies in same zone.
...
**Sign-off**: Ashish Tikhile | 2026-02-17

---

**Next Session**: Backend Service Layer Implementation
